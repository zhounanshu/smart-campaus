...
class DgramServer(BaseServer):
    ...
    def __init__(self, listener, handle=None, backlog=None, spawn='default', **ssl_args):
        BaseServer.__init__(
            self, listener, handle=handle, backlog=backlog, spawn=spawn)
    ...
    def start_accepting(self):
        if self._recv_event is None:
            self._recv_event = core.read_event(
                self.socket.fileno(), self._do_recv, persist=True)
    ...
    def _do_recv(self, event, _evtype):
        ...
        if self.delay >= 0:
            self.stop_accepting()
            self._start_receving_timer = core.timer(
